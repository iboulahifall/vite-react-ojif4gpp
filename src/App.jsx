...
import { LeaseConfig } from "./Billing.jsx";
...
function OwnerApp({ db, onRecord, onProblemStatus, onRepair, onOpenPhoto, onExpenseStatus, onAddExpense, onUploadReceipt, onDeleteExpense, onThreshold, onAddDocument, onUploadDocument, onOpenDocument, onDeleteDocument, onAddProperty, onAddUnit, onUpdateProfile, onMarkNotifsRead, logout }) {
  ...
  return (
    <Shell nav={<BottomNav items={items} active={view.name} onPick={(n) => setView({ name: n })} />}>
      ...
      {view.name === "property" && <PropertyDetail property={props.find((p) => p.id === view.id)} back={() => setView({ name: "properties" })} onInvite={setInvite} onAddUnit={() => setAddUnitFor(view.id)} onInspect={(unit, propId) => setView({ name: "inspections", unitId: unit.id, unitLabel: unit.label, propId })} onConfigLease={(unit, propId) => setView({ name: "leaseconfig", unitId: unit.id, unitLabel: unit.label, propId })} />}
...
function PropertyDetail({ property, back, onInvite, onAddUnit, onInspect, onConfigLease }) {
        ...
        {property.units.map((u) => {
          ...
          return (
            <div key={u.id} style={{ ...card }}>
              ...
              <button onClick={() => onInspect && onInspect(u, property.id)} style={{ ...ghostBtn, width: "100%", justifyContent: "center", marginTop: 8 }}>
                <ClipboardCheck size={15} /> Contrôle logement
              </button>
              <button onClick={() => onConfigLease && onConfigLease(u, property.id)} style={{ ...ghostBtn, width: "100%", justifyContent: "center", marginTop: 8 }}>
                <Receipt size={15} /> Configurer le bail
              </button>
            </div>
          );
        })}
...
function TenantApp({ db, unitId, onRecord, onAddProblem, onUploadPhoto, logout }) {
  ...
  if (screen === "wavepay")
    return <WavePayScreen phone={db.owner.wave_number || db.owner.phone} amount={unit.rent} label={unit.label} back={() => setScreen("home")} />;
    ...
    <Shell>
      <div className="fade" style={{ paddingTop: 24 }}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          {last.status !== "paye" && (db.owner.wave_number || db.owner.phone) && <BigButton icon={Smartphone} label="Payer avec Wave" sub="Paiement mobile instantané" tint="#1DC8F2" onClick={() => setScreen("wavepay")} />}
